...
Code for parsing and cleaning data as well as the logistic regressions is shown at https://github.com/jagerhartman1/EDAV_Project
...
p3 <-ggplot(data=nvy_bi, aes(x=reorder(No.Features, -No.Coef), y=No.Coef)) + geom_bar(stat="identity", fill = rgb(1,0,1), alpha=0.5) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + labs(title='Voted No', x='Bigram', y='Coefficient') + ylim(0,3.1)


p4 <- ggplot(data=nvy_bi, aes(x=reorder(Yes.Features, -Yes.Coef), y=Yes.Coef)) + geom_bar(stat="identity", fill = rgb(0, 1, 0), alpha=0.5) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + labs(title='Voted Yes', x='Bigram', y='Coefficient') + ylim(0,3.1)
...
Politicians don't say much of anything when on the floor of Congress.  There is no clear intellectual debate that occurs that the people who voted for these Congressmen may be hopeful of.  There also doesn't appear to be a common theme or a common dialectic occur in Congress.  The majority of indicators towards sentiment and political offiliation occur through political formalities and who the politician's connections with other politicians.

### Speech Analysis of Congress Members Who Voted Yes/No
...
In the above diagram we see the most incriminating figure from Task 1 that shows a majority of influence towards a politician's vote is based on who they talk to and formalities.  The coefficients are those learned by a logistic regression classifier.  The higher the coefficient, the more impactful that bigram's presence is on the final prediction.  That is, what politicians are saying on the floor of Congress, with regards to debate, has little impact on their vote.  This is contradictory to the ideal Democratic Republic where meaningful debate occurs with the interest of the population in mind.
